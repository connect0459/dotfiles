...
##### Step 4: 調節を型でどこまで閉じるか

「即時調節」「段階的調節」を選んだ場合、モデルの再構築は多くの場合enumへの値追加で止まる。しかしenumの値を追加しただけでは、「その値が本来伴うべき別のフィールド」が伴わない不正な組み合わせを防げない。これはPhase 1で問題視した二項対立の特権化と同じ構造が、スケールを変えて再発しているにすぎない。

不正な状態の組み合わせをどこまで型で禁止するかは、言語ごとに保証レベルが異なる。同じ「型で閉じる」という記述でも、言語によって得られる保証が違う点を区別せずに適用すると、保証の有無を偽ることになる。

| 言語 | 保証手段 | 保証レベル |
| :--- | :--- | :--- |
| Rust | `enum` + 網羅的 `match` | コンパイラ保証 |
| TypeScript | discriminated union + `never` チェック | コンパイラ保証 |
| Go | 非公開interfaceメソッドによるsealed模倣 + `switch` | 静的解析（`exhaustive`等）依存、コンパイラ保証なし |
| PHP | 慣習またはライブラリ | 実行時検証のみ |

保証レベルが高いほど実装コストも上がるため、Step 2の調節コスト試算（影響エンティティ数・テスト書き換え範囲・チーム合意コスト）と合わせて、どこまで型で閉じるかを判断する。非プロトタイプ的ケースの発生頻度が低い場合は、既存のenumのまま「監視継続」で構わない。実装例は `agent-docs/examples/domain-objects.md` の「型による閉じ込め」を参照。
...
```text
即時調節・段階的調節を選んだ場合、再構築後のモデルで発生しうる不正な状態の組み合わせを列挙せよ。
対象言語の型システムでその組み合わせをどこまで排除できるか（コンパイラ保証／静的解析依存／実行時検証のみ）を明示し、
保証レベルと実装コストのトレードオフを踏まえた採用可否を示せ。
```
